const { Plugin, openTab, Dialog, showMessage, confirm } = require('siyuan');
    ...
    onload() {
        ...
        this.tab = this.addTab({
            ...
            init() {
                ...
                this.element.innerHTML = `<div class="fn__flex fn__flex-1 fn__flex-column"><iframe src="/plugins/siyuan-plugin-excalidraw/index.html" style="border: none" class="excalidraw excalidraw-wrapper fn__flex fn__flex-1" data-name="${this.data.name}" data-id="${id}"></iframe></div>`;
                const contentWindow = this.element.querySelector('.excalidraw').contentWindow;
                plugin.windows[id] = contentWindow;
            },
            destroy() {
                tabs.splice(tabs.findIndex((v) => v.name === this.data.name), 1);
            }
        });
    }
...
<div class="fn__flex-1 fn__flex-column">
        ...
        ${this.data.files?.map((file) => {
                ...
                <span class="fileicon deletefile b3-tooltips b3-tooltips__s" aria-label="删除" data-name="${file}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>
                    ...
                    for (const fileEl of files) {
                        ...
                        fileEl.querySelector('.copyfile').addEventListener('click', (event) => {
                            event.preventDefault();
                            event.stopPropagation();
                            copyDraw(fileEl.getAttribute('data-name'));
                        });
                        fileEl.querySelector('.editfile').addEventListener('click', (event) => {
                            event.preventDefault();
                            event.stopPropagation();
    ...
    copyDraw(filename) {
        ...
        const link = `siyuan://plugins/${pluginName}${tabType}?icon=iconExcalidraw&title=${filename}&data=${JSON.stringify({ name: filename })}`;
